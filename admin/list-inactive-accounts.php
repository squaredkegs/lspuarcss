...
<html>
  <head>
	...
	<script type="text/javascript">
		function button_action(this_button,e){
			var sid = $(this_button).attr("id");
			var name = $(this_button).attr("name");
			var form = $(this_button).closest("form");
			var other_button = $(form).find("button:eq(0)");
			var second_button = $(form).find("button:eq(1)");
			e.preventDefault();
			if(name=='reactivated'){
				$.ajax({
					type: 'POST',
					url: 'php/reactivate_account.php',
					cache: false,
					data: 
					{
						sid: sid,
					...
					success: function(data){
						$(form).html("<button class='btn' style='color:white;background-color:grey;' onclick='undo_button(this,event);' id='" + sid + "'>Undo</button>");
					},
				});
			}
				...
				$.ajax({
					type: 'POST',
					url: 'php/make_account_inactive.php',
					cache: false,
					data:
					{
						sid: sid,
		...
		function undo_button(this_button,e){
			...
			$.ajax({
				...
				success: function(data){
					$(form).html("<button class='reactivate btn btn-info' id='" + sid + "' name='reactivated' onclick='button_action(this,event);'>Reactivate</button> <button class='btn btn-danger' id='" + sid + "' name='removed' onclick='button_action(this,event);'>Remove from list</button>");
		...
		function show_confirmation_reactivation(this_button,e){
			var div = $("#reactivate_all_div").html("<label style='display:block;'>Are You Sure?</label><button class='btn btn-info' onclick='reactivate_all(this,event);'>Yes</button><button class='btn btn-danger' style='margin-left:10px;' onclick='not_reactivate(this,event);'>No</button>")
			e.preventDefault();
		}
    ...
    <div class="wrapper">
      ...
      <div class="content-wrapper" style="background-color:#DCDCDC;">
				  <div class="box"  >
					...
					<a href='list-expired-accounts' style='display:block;margin-top:10px;'><button class='btn btn-info'>Expired Accounts</button></a>
					</div>
					...
					<div class="box-body" >
					  ...
					  <table id="example1" class="table table-bordered table-striped">
						...
						<?php
							...
							$query = $db -> prepare ("SELECT fname,lname,status,stud_id FROM stud_bas WHERE campus = :campus AND department = :department AND (status = 'Inactive') AND (lname >= :first_letter AND lname < char(ascii(:second_letter) + 1)) ORDER BY lname ASC");
